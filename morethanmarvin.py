...
async def handler(bot, event):
    ...
    if str(event.msg.content.text.body).startswith("!bible"):
        ...
        # Get everything after "!bible "
        args = str(event.msg.content.text.body)[7:].strip()

        # If user wrote something like: "!bible John 3:16 NIV"
        # Last "word" is version if it's all letters and <= 10 chars
        parts = args.rsplit(" ", 1)
        if len(parts) == 2 and parts[1].isalpha() and len(parts[1]) <= 10:
            passage = parts[0]
